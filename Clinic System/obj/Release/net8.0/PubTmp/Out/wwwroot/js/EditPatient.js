...
document.addEventListener('DOMContentLoaded', function () {
	...
	const specialCasesContainer = document.getElementById('specialCasesContainer');

	// Event listener for removing chronic diseases
	chronicDiseasesContainer.addEventListener('click', function (event) {
		if (event.target.classList.contains('remove-ChronicDisease')) {
			const itemToRemove = event.target.closest('.mb-3');
			if (itemToRemove) {
				chronicDiseasesContainer.removeChild(itemToRemove);
			}
		}
	});

	// Event listener for removing special cases
	specialCasesContainer.addEventListener('click', function (event) {
		if (event.target.classList.contains('remove-SpecialCase')) {
			const itemToRemove = event.target.closest('.mb-3');
			if (itemToRemove) {
				specialCasesContainer.removeChild(itemToRemove);
			}
		}
	});
});
